# dotfiles
My Manjaro install's dotfiles and other configurations.

## Main Setup

- [Manjaro](https://www.manjaro.org/)
- [i3-gaps](https://github.com/Airblader/i3)
- [ibhagwan's picom](https://github.com/ibhagwan/picom)
- [dunst](https://github.com/dunst-project/dunst)
- [polybar](https://github.com/jaagr/polybar)
- [rofi](https://github.com/DaveDavenport/rofi)
- [feh](https://github.com/derf/feh)
- [alacritty](https://st.suckless.org/)
- [zsh](https://www.zsh.org/)
  - [oh-my-zsh](https://github.com/ohmyzsh/ohmyzsh)
  - [zsh-autocomplete](https://github.com/zsh-users/zsh-autosuggestions)
  - [dotbare](https://github.com/kazhala/dotbare)
- lightdm-webkit2-greeter
  - [Glorious Theme](https://github.com/manilarome/lightdm-webkit2-theme-glorious)
- [pywal](https://github.com/dylanaraps/pywal)

- [Firefox](https://firefox.com)
- [Visual Studio Code](https://code.visualstudio.com/)
- [Spotify](https://github.com/ncmpcpp/ncmpcpp)

### KDE Configurations
- DE: KDE Plasma
- Global Theme: Sweet Mars
- Application Style: [Lightly](https://github.com/Luwx/Lightly)
- Plasma Style: [WhiteSur-dark](https://store.kde.org/p/1400424/)
- Colors: [Lightly](https://github.com/manilarome/the-glorious-plasma-setup/tree/master/local/share/color-schemes)
- Window Decorations: [BreezeBlurred](https://github.com/alex47/BreezeBlurred)
- Fonts: Inter, Fira Mono
- Icons: [La Capitaine](https://github.com/keeferrourke/la-capitaine-icon-theme)
- Cursor: macOSBigSur
- Splash Screen: None
- Others : [LightlyShaders](https://github.com/Luwx/LightlyShaders)
